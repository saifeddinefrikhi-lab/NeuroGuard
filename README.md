...
| **Gateway Service** | 8083 | Spring Cloud Gateway, Keycloak JWT | API routing, CORS, load balancing |
...
| **Risk Alert Service** | 8084 | Node.js, Express, WebSocket | Alert generation (rules & predictive), notifications |
...
- **Gateway** – Routes requests to appropriate services, adds CORS headers, validates Keycloak JWT tokens, and forwards the authorization header.
- **Config Server** – Centralizes shared configuration for the gateway and backend services.
...
- **Risk Alert Service** – Node.js alert engine that evaluates patient data against rules and predictive scoring, then generates alerts and notifications.
...
### 📦 Gateway Service
- **Port:** `8083`
- **Purpose:** API Gateway with dynamic routing, CORS, and Keycloak JWT validation.
- **Routes:**
  | Path | Target Service |
  |------|----------------|
  ...
  | `/alerts/**`, `/api/patient/alerts/**`, `/api/caregiver/alerts/**`, `/api/provider/alerts/**`, `/api/notifications/**` | risk-alert-service |
- **Config Source:** `http://localhost:8888`
- **Run:** `mvn spring-boot:run`

### 📦 Config Server
- **Port:** `8888`
- **Purpose:** Serves shared configuration for the gateway and Spring services.
- **Config Repo:** `neuroguard-backend/config-server/config-repo`
- **Run:** `mvn spring-boot:run`
...
- **Storage:** Local JSON store by default, ready to be swapped with a database if needed
- **Alert Generation:**
  - **Rule‑based:** medical risk rules such as severe progression, age, comorbidities, allergies, and low assessment scores
  - **Predictive:** accepts a probability or features payload and raises an alert when the threshold is crossed
  - **Manual:** providers can create alerts via API
- **Notifications:** in-app notifications for patients and assigned caregivers through REST and WebSocket events
- **Key Endpoints:**
  - `POST /api/provider/alerts/generate` – rule-based generation
  - `POST /api/provider/alerts/generate-predictive` – predictive generation
...
5. **Run the Config Server** (in a separate terminal):
  ```bash
  cd config-server
  mvn spring-boot:run
  ```

6. **Run the other microservices** (in separate terminals):
   ```bash
   cd user-service && mvn spring-boot:run
   cd medical-history-service && mvn spring-boot:run
  cd risk-alert-service && npm install && npm start
  cd gateway && mvn spring-boot:run
   ```

7. **Start the ML predictor**
   ```bash
